...
const SPAWN_POINT: Vec3 = Vec3::new(-0.150, 1.625, 0.0);
const CT_SPAWN : Vec3 = Vec3::new(5.14, -1.61, -61.62);
...
fn setup(mut commands: Commands, mut window: Query<&mut Window>, assets: Res<AssetServer>) {
    ...
    window.title = String::from("League of over duty global offensive");
    ...
    let logical_entity = commands
        .spawn((
            ...
            TransformBundle::from_transform(Transform::from_translation(CT_SPAWN)),
            LogicalPlayer,
            FpsControllerInput {
                pitch: -TAU / 12.0,
                yaw: TAU * 5.0 / 8.0,
    ...
    // Camera setup with AK-47 model parented to the camera
    ...
    // Load AK-47 model and parent it to the camera
    let ak47_handle = assets.load("./weapons/ak47.glb#Scene0");

    // Spawn AK-47 as a child of the camera, adjusting its transform
    commands.entity(camera_entity).with_children(|parent| {
        parent.spawn(SceneBundle {
            scene: ak47_handle.clone(),
            transform: Transform {
                translation: Vec3::new(0.5, -0.6, -1.0),  // Adjust the position to appear in front of the camera
                rotation: Quat::from_rotation_y(-0.1),   // Slight rotation to orient it naturally
                scale: Vec3::splat(0.5),                 // Adjust the scale as necessary
                ..default()
            },
            ..default()
        });
    ...
    commands.insert_resource(MainScene {
        handle: assets.load("./maps/dust.glb"),
        ak_handle: ak47_handle,
        is_loaded: false,
    });
    ...
    commands.spawn(
        TextBundle::from_section(
            "",
            TextStyle {
                font: assets.load("./fonts/fira_mono.ttf"),
                font_size: 24.0,
                color: Color::BLACK,
            },
        )
...
fn respawn(mut query: Query<(&mut Transform, &mut Velocity)>) {
    for (mut transform, mut velocity) in &mut query {
        ...
        transform.translation = CT_SPAWN;
    }
}
